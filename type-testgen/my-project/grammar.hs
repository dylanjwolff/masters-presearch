...
happyExpList = Happy_Data_Array.listArray (0,80) ([0,34,8192,32,0,0,0,8226,0,33024,26,1024,33024,16410,23169,0,0,6785,33024,26,0,0,0,0,33216,90,0,33024,26,6785,33024,16410,24193,33088,91,0,0,0,0,33088,90,16384,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parser","FDecl","TypeExp","'::'","'->'","'=>'","type","class_decl","type_decl","data_decl","'='","where","'|'","'['","']'","'('","')'","'*'","name","fname","','","%eof"]
        bit_start = st * 24
        bit_end = (st + 1) * 24
...
action_12 (9) = happyShift action_10
...
action_22 (5) = happyGoto action_17
...
action_25 (5) = happyGoto action_17
...
happyNewToken action sts stk (tk:tks) =
	...
	TokenColons -> cont 6;
	TokenArrow -> cont 7;
	TokenBigArrow -> cont 8;
	TokenType happy_dollar_dollar -> cont 9;
	TokenClassMeta -> cont 10;
	TokenTypeMeta -> cont 11;
	TokenDataMeta -> cont 12;
	TokenEquals -> cont 13;
	TokenWhere -> cont 14;
	TokenOr -> cont 15;
	TokenOSB -> cont 16;
	...
	TokenCB -> cont 19;
	TokenAny -> cont 20;
	TokenName happy_dollar_dollar -> cont 21;
	TokenFName happy_dollar_dollar -> cont 22;
	TokenComma -> cont 23;
	_ -> happyError' ((tk:tks), [])
	}
